...
\begin{frame}{When Is the Projected Operator Stable?}
  Let $\xi_\pi$ be a fully supported invariant distribution under $\pi$.
  \begin{block}{Projected Bellman contraction (Bellemare et al., 2023, Theorem 9.8}
    Since $\Pi_{\phi,\xi_\pi}$ is nonexpansions in
    $\norm{\cdot}_{\xi_\pi,2}$,
    \[
  ...
  \end{block}
  \pause
  \textcolor{red}{How good is this compared to the true best value function?}
  \pause
...
\begin{frame}{Section 9.4: Semi-Gradient TD}
  ...
  \]
  \begin{block}{Fixed point and TD relation}
...
\begin{frame}{Quantile Functions}
  \centering
  \begin{tikzpicture}[x=0.92cm,y=0.92cm,>=Stealth,
    ...
    \draw[very thick,black]
      plot[smooth] coordinates {(0.35,0.12) (1.25,0.35) (2.25,0.75)
      (3.35,1.55) (4.05,2.35) (5.45,3.15) (7.4,3.78)};
    \node[black] at (6.2,3.55) {$F_{\eta(x)}$};

    \draw[densely dashed,blue!65!black] (0,0.75) -- (2.25,0.75) -- (2.25,0);
    \fill[blue!65!black] (2.25,0.75) circle (1.8pt);
    \node[left,blue!65!black] at (0,0.75) {$\tau_1$};
    \node[below,blue!65!black] at (2.25,0) {$\theta_1(x)$};

    \draw[densely dashed,red!75!black] (0,1.55) -- (3.35,1.55) -- (3.35,0);
    \fill[red!75!black] (3.35,1.55) circle (1.8pt);
    \node[left,red!75!black] at (0,1.55) {$\tau_2$};
    \node[below,red!75!black] at (3.35,0) {$\theta_2(x)$};

    \draw[densely dashed,teal!70!black] (0,2.35) -- (4.05,2.35) -- (4.05,0);
    \fill[teal!70!black] (4.05,2.35) circle (1.8pt);
    \node[left,teal!70!black] at (0,2.35) {$\tau_3$};
    \node[below,teal!70!black] at (4.05,0) {$\theta_3(x)$};

    \draw[densely dashed,orange!80!black] (0,3.15) -- (5.45,3.15) -- (5.45,0);
    \fill[orange!80!black] (5.45,3.15) circle (1.8pt);
    \node[left,orange!80!black] at (0,3.15) {$\tau_4$};
    \node[below,orange!80!black] at (5.45,0) {$\theta_4(x)$};
    ...
    \begin{scope}[yshift=-1.75cm]
      ...
      \draw[very thick,blue!65!black] (2.25,0) -- (2.25,0.55);
      \fill[blue!65!black] (2.25,0.55) circle (1.7pt);
      \draw[very thick,red!75!black] (3.35,0) -- (3.35,0.55);
      \fill[red!75!black] (3.35,0.55) circle (1.7pt);
      \draw[very thick,teal!70!black] (4.05,0) -- (4.05,0.55);
      \fill[teal!70!black] (4.05,0.55) circle (1.7pt);
      \draw[very thick,orange!80!black] (5.45,0) -- (5.45,0.55);
      \fill[orange!80!black] (5.45,0.55) circle (1.7pt);
...
\begin{frame}{Distributional TD: Linear Learning}
...
\begin{frame}{Linear CTD: How to Stay on Fixed Support}
  ...
  the categorical projection $\Pi_c$ to project the shifted atoms back to 
...
\section{Theory when allowing signed measures}

\begin{frame}{Signed Categorical Family}
...
\begin{frame}{Linear Approximation of signed measures}
...
\begin{frame}{Why Two Projections?}
  \small
...
\begin{frame}{The Cram\'er Distance}
...
\begin{frame}{Bellman and Categorical Projections}
  \small
  Assume the MDP has stationary distribution $\xi_\pi$ under policy $\pi$.
  \begin{block}{Lemma 9.14: Bellman Step}
    Under these assumptions,
      $\T^\pi:\mathcal M_{\ell_2,1}(\R)^\X
      \to
      \mathcal M_{\ell_2,1}(\R)^\X$
    is a $\gamma^{1/2}$-contraction with respect to $\ell_{\xi_\pi,2}$:
    \[
  ...
  \begin{block}{Lemma 9.15: Categorical Projection}
    The pointwise categorical projection operator
    \[
      \Pi_c:\mathcal M_{\ell_2,1}(\R)^\X\to\F_{S,m}^{\X},
      \qquad
      (\Pi_c\eta)(x)=\Pi_c(\eta(x))
    \]
    is a nonexpansion with respect to $\ell_{\xi_\pi,2}$:
    \[
      \ell_{\xi_\pi,2}(\Pi_c\eta,\Pi_c\eta')
      \leq
...
\begin{frame}{Feature Projection}
  ...
  \begin{block}{Lemma 9.16: Linear Feature Projection}
    The function approximation projection
    $
      \Pi_{\phi,\xi_\pi,\ell_2}:\mathcal F_{S,m}^{\X}
      \to
      \mathcal F_{\phi,S,m}
    $
    is a nonexpansion with respect to $\ell_{\xi_\pi,2}$:
...
\begin{frame}{Theorem: Doubly Projected Bellman}
  Composing the three preceding Lemma gives us the main result.
...
\begin{frame}{From Fixed Features to Learned Features}
  A deep network replaces the hand crafted features $\phi$ by a learned
  representation. Its penultimate layer can still be read as
  \[
    \phi_w(x, a)\in\R^n,
  \]
  followed by linear prediction layer outputting the final things we are interested in.
\end{frame}
...
\begin{frame}{From DQN to Distributional DQN}
  ...
  \]
  \pause
  Here $w$ are the weights we update. The weights
  $\widetilde w$ are a frozen target-network copy, used to have a fixed target. \\
...
\begin{frame}{QR-DQN: Quantile Prediction}
  ...
  \[
    ...
    -\theta_i(x,a;w).
  \]
  Every predicted quantile is compared with every target quantile. We are intereted in how much of the target is below/above the quantile.
\end{frame}
...
\begin{frame}{Huber vs Linear Quantile loss}
  \begin{figure}
    \centering
    \begin{minipage}{0.48\linewidth}
      \centering
      \includegraphics[width=\linewidth]{images/2026-07-01-11-07-03.png}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\linewidth}
      \centering
      \includegraphics[width=\linewidth]{images/2026-07-01-11-07-18.png}
    \end{minipage}
  \end{figure}
  \vspace{-0.8em}
  \small
  For residual $u=y-\theta_i$, $u>0$ means the target lies above the
  predicted quantile, so the prediction is too low. High quantiles therefore
  give positive residuals a large weight $\tau_i$; low quantiles give them a
  small weight. Negative residuals get the opposite weight $1-\tau_i$.
\end{frame}

\section{Particle DQN}

\begin{frame}{Particle DQN}
  \small
  QR-DQN represents
  \[
    \eta_w(x,a)=\frac1m\sum_{i=1}^m\delta_{\theta_i(x,a;w)}
  \]
  where $\theta_i$ are quantile levels. MMD-DQN and
  SinkhornDRL keep the same empirical-measure form
  \[
    \eta_w(x,a)=\frac1m\sum_{i=1}^m\delta_{z_i(x,a;w)},
  \]
  and treat the $z_i$ as unrestricted deterministic samples.

  \begin{block}{Overview of Particle DQN}
    \begin{enumerate}
      \item Define a metric $d$ between return distributions.
      \item Study whether $\T^\pi$ contracts under
  ...
  \pause
  \begin{center}
    \textcolor{red}{The metric NEEDS to be differentiable in the particles, this DOES NOT hold for the standard Wasserstein.}
  \end{center}

\end{frame}

\begin{frame}{MMD-DQN: Maximum Mean Discrepancy}
  \small
  Let $\mathcal H_k$ be the RKHS of a continuous kernel $k$ and let
  \[
    \psi_\mu=\int k(z,\cdot)\,d\mu(z)\in\mathcal H_k
  \]
  ...
  \[
    ...
    =
    \E k(Z,Z')+\E k(Y,Y')-2\E k(Z,Y).
  \]
  \begin{block}{Characteristic kernel}
    $k$ is characteristic if the embedding $\mu\mapsto\psi_\mu$ is injective:
    \[
      \psi_\mu=\psi_\nu \quad\Longrightarrow\quad \mu=\nu.
    \]
    For all such $k$ the MMD is a metric.
  \end{block}
\end{frame}

\begin{frame}{MMD-DQN: Supremum Metric and Contraction}
  \small
  Nguyen et al. define the distributional-RL metric
  \[
    \operatorname{MMD}_{\infty,k}(\eta,\nu)
    =
    \sup_{x,a}\operatorname{MMD}_k(\eta(x,a),\nu(x,a)).
  \]
  Their contraction result requires more than ``$k$ is characteristic.'' A
  kernel must be:
  \[
    k(x+c,y+c)=k(x,y)
    \quad\text{and}\quad
    \operatorname{MMD}_k((b_{\gamma,0})_\#\mu,(b_{\gamma,0})_\#\nu)
    =
    \gamma^{\alpha/2}\operatorname{MMD}_k(\mu,\nu).
  \]
  Here $b_{\gamma,0}(z)=\gamma z$ and $\alpha$ is the scale-sensitivity order.

  \begin{block}{Theorem idea}
    For mixtures of such kernels, the distributional Bellman operator is
    \[
      \operatorname{MMD}_{\infty,k}(\T^\pi\eta,\T^\pi\nu)
      \leq
      \gamma^{\alpha/2}
      \operatorname{MMD}_{\infty,k}(\eta,\nu).
    \]
  \end{block}
  So $\T^\pi$ has the same fixed-point story as in Wasserstein, but only under
  suitable MMD kernels.
\end{frame}

\begin{frame}{MMD-DQN: Empirics are weird}
  \small
  Theory and empirical observations do not align.
  \begin{itemize}
    \item Unrectified kernels such as
      \[
        k_\alpha(x,y)=-\|x-y \|^\alpha,
      \]
      fulfill the assumption of the Theorem.
    \item They prove that $\T^\pi$ is \emph{not} a contraction in
      MMD for Gaussian kernels in general. \\
  \end{itemize} 

  Yet, in practice the Gaussian Kernel yields better results. 
\end{frame}

\begin{frame}{MMD-DQN: Learning with Particles}
  \small
  \setlength{\tabcolsep}{0.35em}
  \begin{center}
  \begin{tabular}{p{0.08\textwidth}p{0.84\textwidth}}
    \hline
    \multicolumn{2}{l}{\textbf{Algorithm: MMD-DQN particle update}}\\
    \hline
    \multicolumn{2}{l}{\textbf{Require:} number of particles $m$, kernel $k$,
      discount $\gamma$, target weights $\widetilde w$}\\
    \multicolumn{2}{l}{\textbf{Input:} sample transition $(x,a,r,x')$}\\[0.2em]
    1 & Network outputs current particles
      $Z_w(x,a)=\{z_i(x,a;w)\}_{i=1}^m$,
      $\eta_w(x,a)=\frac1m\sum_i\delta_{z_i(x,a;w)}$.\\[0.3em]
    2 & Select greedy next action
      \[
        a^*\in\arg\max_{a'}\frac1m\sum_{j=1}^m
        z_j(x',a';\widetilde w).
      \]\\[-0.4em]
    3 & Construct Bellman target particles
      \[
        y_j=r+\gamma z_j(x',a^*;\widetilde w),
        \qquad 1\leq j\leq m.
      \]\\[-0.4em]
    4 & \textbf{Output:} Empirical MMD Bellman loss
      \[
        L_{\operatorname{MMD}}(w)
        =
        \frac1{m^2}\sum_{i,i'}k(z_i,z_{i'})
        +
        \frac1{m^2}\sum_{j,j'}k(y_j,y_{j'})
        -
        \frac2{m^2}\sum_{i,j}k(z_i,y_j).
      \]\\[-0.4em]
    \hline
  \end{tabular}
  \end{center}
...
\begin{frame}{SinkhornDRL: Regularized Wasserstein Loss}
  \small
  ...
  \[
    \operatorname{OT}^{\varepsilon}_c(\mu,\nu)
    =
    \inf_{\pi\in\Pi(\mu,\nu)}
    \int c\,d\pi
    +
    \varepsilon\,\operatorname{KL}(\pi\mid\mu\otimes\nu).
  \]
  The sinkhorn divergence is then given by 
  \[
    \overline W_{c,\varepsilon}(\mu,\nu)
    =
    \operatorname{OT}^{\varepsilon}_c(\mu,\nu)
    -\frac12\operatorname{OT}^{\varepsilon}_c(\mu,\mu)
    -\frac12\operatorname{OT}^{\varepsilon}_c(\nu,\nu).
  \]
\end{frame}

\begin{frame}{Reguralized Wasserstein}
  \begin{figure}[h]
\centering
\includegraphics[width=0.8\linewidth]{images/2026-07-01-13-37-43.png}
\caption{Comparison of different regularized Wasserstein distances. }
\label{fig:main-2026-07-01-13-37-43}
\end{figure}
\end{frame}

\begin{frame}{SinkhornDRL: Contraction Result}
  \small
  Sun et al. use the standard cost $
    c(x,y)=\|x-y\|^\alpha,$
  and define
  \[
    \overline W^\infty_{c,\varepsilon}(\eta,\nu)
    =
    \sup_{x,a}\overline W_{c,\varepsilon}(\eta(x,a),\nu(x,a)).
  \]
  With
  \[
    \lambda_\varepsilon(\mu,\nu)
    =
    \frac{\varepsilon\,\operatorname{KL}(\pi^*\mid\mu\otimes\nu)}
         {\overline W_{c,\varepsilon}(\mu,\nu)}
    \in(0,1),
  \]
  the Bellman operator is at least
  \[
    \overline\Delta_\varepsilon(\gamma,\alpha)
    =
    \gamma^\alpha\!\left(1-\sup_{\mu,\nu}\lambda_\varepsilon(\mu,\nu)\right)
    +\sup_{\mu,\nu}\lambda_\varepsilon(\mu,\nu)
    \in(\gamma^\alpha,1)
  \]
  contractive under $\overline W^\infty_{c,\varepsilon}$.
\end{frame}

\begin{frame}{SinkhornDRL: Learning with Particles}
  \small
  \setlength{\tabcolsep}{0.35em}
  \begin{center}
  \begin{tabular}{p{0.08\textwidth}p{0.84\textwidth}}
    \hline
    \multicolumn{2}{l}{\textbf{Algorithm: SinkhornDRL particle update}}\\
    \hline
    \multicolumn{2}{l}{\textbf{Require:} number of particles $m$, cost $c$,
      regularization $\varepsilon$, discount $\gamma$, target weights $\widetilde w$}\\
    \multicolumn{2}{l}{\textbf{Input:} sample transition $(x,a,r,x')$}\\[0.2em]
    1 & Network outputs current particles
      $Z_w(x,a)=\{z_i(x,a;w)\}_{i=1}^m$,
      $\eta_w(x,a)=\frac1m\sum_i\delta_{z_i(x,a;w)}$.\\[0.3em]
    ...
    3 & Construct Bellman target particles
      \[
        ...
        \qquad 1\leq j\leq m.
      \]\\[-0.4em]
    4 & \textbf{Output:} approximate Wasserstein using Sinkhorn:
      \[
        L_{\operatorname{RW}}(w)
        =
        {\overline W}_{c,\varepsilon}
        \!\left(
          \frac1m\sum_i\delta_{z_i(x,a;w)},
          \frac1m\sum_j\delta_{y_j}
        \right).
      \]\\[-0.4em]
    \hline
  \end{tabular}
...
\begin{frame}{SinkhornDRL: Interpolating Wasserstein and MMD}
  \small
  The regularization parameter moves between Wasserstein and MMD:
  \[
    \varepsilon\to0
  ...
  \[
    ...
    \quad\Rightarrow\quad
    \overline W_{c,\varepsilon}(\mu,\nu)\to
    \operatorname{MMD}_{k_\alpha}^2(\mu,\nu),
    \qquad k_\alpha=-c.
  \]
  ...
  \begin{tabular}{p{0.15\textwidth}p{0.25\textwidth}p{0.23\textwidth}p{0.28\textwidth}}
    \hline
    \textbf{Algorithm} & \textbf{Distribution divergence} &
    \textbf{Representation $Z_\theta$} & \textbf{Rate of $\T^\pi$}\\
    \hline
    C51 & Cramer distance & Categorical distribution & $\sqrt{\gamma}$\\
    QR-DQN-1 & Wasserstein distance & Quantiles & $\gamma$\\
    MMD-DQN & MMD & Samples & $\gamma^{\alpha/2}$ $(k_\alpha)$\\
    SinkhornDRL & Sinkhorn divergence & Samples &
      $\gamma$ $(\varepsilon\to0)$,
      $\gamma^{\alpha/2}$ $(\varepsilon\to+\infty)$\\
    \hline
  \end{tabular}
\end{frame}

\begin{frame}{SinkhornDRL: Atari Comparison}
  \begin{figure}[h]
    \centering
    \includegraphics[width=1\linewidth]{images/2026-07-01-14-35-50.png}
    \caption{Comparison of various algorithms on 55 Atari games.}
    \label{fig:main-2026-07-01-14-35-50}
  \end{figure}
\end{frame}


\iffalse
\section{IQN and Risk-Sensitive Prediction}
...
\begin{frame}{Takeaways}
  \begin{enumerate}
    ...
    \item MMD-DQN and SinkhornDRL are metric-first particle methods: they define
      a distribution distance, analyze Bellman contraction, then use the
      empirical distance as the DQN loss.
  \end{enumerate}
\end{frame}
...
\begin{frame}{References}
  \footnotesize
  \begin{itemize}
    \item Bellemare, Dabney, Rowland (2023): \emph{Distributional Reinforcement Learning}
    \item Nguyen et al. (2020): Distributional Reinforcement Learning via Moment Matching.
    \item Sun et al. (2022): Distributional Reinforcement Learning with Regularized Wasserstein Loss.
  \end{itemize}
\end{frame}
