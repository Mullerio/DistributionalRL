...
\begin{frame}{Linear Signed Approximation of signed measures}
...
\begin{frame}{Bellman and Categorical Projections}
  \small
  Assume rewards have finite first moments and the Markov chain under $\pi$
  has a unique stationary distribution $\xi_\pi$.
...
\begin{frame}{Particle Distributional DQN}
  \small
  QR-DQN represents
  \[
    \eta_w(x,a)=\frac1m\sum_{i=1}^m\delta_{\theta_i(x,a;w)}
  ...
  \begin{block}{Overview of Particle Distributional DQN}
    \begin{enumerate}
      \item Define a divergence $d$ between return distributions.
...
\begin{frame}{MMD-DQN: Maximum Mean Discrepancy}
  ...
  \]
  \begin{itemize}
    \item If $k$ is characteristic, MMD is a metric on probability measures.
    \item Gaussian kernels are characteristic, so they can distinguish
      distributions.
    \item The metric geometry depends completely on the kernel.
  \end{itemize}
\end{frame}
...
\begin{frame}{MMD-DQN: Important Caveat}
  \small
  The paper separates theory and best empirical practice:
  \begin{itemize}
    \item Unrectified kernels such as
      \[
        k_\alpha(x,y)=-|x-y|^\alpha,\qquad 0<\alpha<2,
      \]
      give the clean contraction theorem.
    \item Gaussian kernels are characteristic and useful for moment matching.
    \item But the paper proves that $\T^\pi$ is \emph{not} a contraction in
      MMD for Gaussian kernels in general.
  \end{itemize}

  \begin{block}{Interpretation}
    MMD-DQN uses Gaussian-kernel MMD because it gives a practical particle loss:
    it attracts particles to Bellman target samples and repels particles from
    collapsing. The contraction theorem motivates the metric framework, but
    does not literally certify the Gaussian version used in Atari experiments.
  \end{block}
\end{frame}
...
\begin{frame}{MMD-DQN: Learning with Particles}
  ...
  \]
  Thus the metric is not only an evaluation tool: it becomes the backpropagated
  Bellman error between two empirical return distributions.
\end{frame}
...
\begin{frame}{SinkhornDRL: Regularized Wasserstein Loss}
  ...
  \]
  It is a regularized Wasserstein-type loss: smoother than exact OT and
  computable by Sinkhorn iterations.
\end{frame}
...
\begin{frame}{SinkhornDRL: Contraction Result}
  ...
  \[
    \bar S^\varepsilon_c(\eta,\nu)
    =
    \sup_{x,a}S^\varepsilon_c(\eta(x,a),\nu(x,a)).
  \]
  The paper analyzes contraction through two properties:
  \begin{itemize}
    \item \textbf{Sum invariance:} adding the same independent reward noise does
      not increase the distance.
    \item \textbf{Scale sensitivity:} discounting returns shrinks the distance.
  \end{itemize}
  For the cost family used in their theorem, the distributional Bellman operator
  satisfies a bound of the form
  \[
    \bar S^\varepsilon_c(\T^\pi\eta,\T^\pi\nu)
    \leq
    \rho_{\varepsilon,\gamma}
    \bar S^\varepsilon_c(\eta,\nu),
    \qquad
    \rho_{\varepsilon,\gamma}<1.
  \]
  The constant depends on $\gamma$, the regularization $\varepsilon$, the cost
  order, and the return-distribution set of the finite MDP.
\end{frame}

\begin{frame}{SinkhornDRL: Interpolating Wasserstein and MMD}
  \small
  The paper emphasizes that Sinkhorn divergence interpolates between two
  familiar geometries:
  \[
    \varepsilon\to0
    \quad\Rightarrow\quad
    S^\varepsilon_c \text{ behaves like Wasserstein/OT},
  \]
  \[
    \varepsilon\to\infty
    \quad\Rightarrow\quad
    S^\varepsilon_c \text{ behaves like an MMD-type distance}.
  \]
  Correspondingly, their theorem recovers contraction behavior consistent with
  both endpoints, and for finite $\varepsilon$ proves a genuine contraction with
  a factor between the Wasserstein contraction factor and $1$.

  \begin{block}{Why this matters}
    Regularization makes the transport plan smoother and the loss easier to
    optimize, but it weakens the contraction factor compared with exact
    Wasserstein. The point is controlled trade-off, not free improvement.
  \end{block}
\end{frame}
...
\begin{frame}{SinkhornDRL: Learning with Particles}
  ...
  \[
    C_{ij}=c(z_i,y_j).
  \]
  With uniform weights $a=b=(1/m,\ldots,1/m)$, Sinkhorn iterations approximate
  \[
    \min_{\Pi\in U(a,b)}
  ...
  \]
  followed by the self-cost debiasing terms in $S^\varepsilon_c$.

  \begin{block}{DQN update}
    Replace the QR-DQN quantile Huber loss by the empirical Sinkhorn divergence
    between current particles and Bellman target particles.
  \end{block}
\end{frame}

\begin{frame}{Metric-Based Particle DQN: Comparison}
  \small
  \begin{center}
  \begin{tabular}{lccc}
    \textbf{Method} & \textbf{Representation} & \textbf{Metric/loss} & \textbf{Theory} \\
    \hline
  ...
  \[
    y_j=r+\gamma z_j(x',a^*;\widetilde w),
    \qquad
    a^*=\arg\max_{a'}\frac1m\sum_jz_j(x',a';\widetilde w).
  \]
  The new idea is that once a distribution metric has a Bellman contraction
  story, its empirical sample version can be used directly as a learning
  objective.
\end{frame}
